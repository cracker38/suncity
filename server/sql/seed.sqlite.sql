...
INSERT OR IGNORE INTO role_permissions (role_id, permission_id) SELECT 8, id FROM permissions;
INSERT OR IGNORE INTO role_permissions (role_id, permission_id) SELECT 2, id FROM permissions WHERE code IN ('bookings.own');
INSERT OR IGNORE INTO role_permissions (role_id, permission_id) SELECT 3, id FROM permissions WHERE code IN ('bookings.manage', 'rooms.manage', 'reports.view');
INSERT OR IGNORE INTO role_permissions (role_id, permission_id) SELECT 4, id FROM permissions WHERE code IN ('restaurant.manage', 'reports.view');
INSERT OR IGNORE INTO role_permissions (role_id, permission_id) SELECT 5, id FROM permissions WHERE code IN ('events.manage', 'reports.view');
INSERT OR IGNORE INTO role_permissions (role_id, permission_id) SELECT 6, id FROM permissions WHERE code IN ('housekeeping.manage', 'catering.manage', 'reports.view');
INSERT OR IGNORE INTO role_permissions (role_id, permission_id) SELECT 7, id FROM permissions WHERE code IN ('finance.manage', 'reports.view');

INSERT OR IGNORE INTO users (id, role_id, email, password_hash, first_name, last_name, phone, is_active, email_verified) VALUES
...
('Free Wi-Fi', 'wifi'), ('Air Conditioning', 'snowflake'), ('Flat-screen TV', 'tv'),
('Private Bathroom', 'bath'), ('Mini Bar', 'wine'), ('Room Service', 'bell'),
('Work Desk', 'desk'), ('Safe', 'lock'), ('Balcony', 'sun'), ('Coffee Maker', 'coffee');

INSERT OR IGNORE INTO room_types (id, name, slug, description, short_description, base_price, max_guests, bed_type, size_sqm, featured, rating_avg, review_count, cover_image) VALUES
(1, 'Standard Room', 'standard-room', 'Comfortable standard accommodation with essential amenities for a restful stay in Nyakarambi.', 'Comfortable essentials for a restful stay', 45000, 2, 'Queen Bed', 22, 1, 4.5, 12, 'https://images.unsplash.com/photo-1611892440504-42a792e24d32?w=1200'),
(2, 'Deluxe Room', 'deluxe-room', 'Spacious deluxe room featuring refined interiors, premium bedding, and scenic views.', 'Refined comfort with premium bedding', 65000, 2, 'King Bed', 30, 1, 4.7, 18, 'https://images.unsplash.com/photo-1590490360182-c33d57733427?w=1200'),
(3, 'Twin Room', 'twin-room', 'Practical twin room with two single beds, perfect for colleagues or friends.', 'Two beds for comfortable shared stays', 55000, 2, 'Twin Beds', 26, 1, 4.4, 9, 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=1200'),
(4, 'Family Room', 'family-room', 'Generous family room designed for groups and families.', 'Spacious comfort for the whole family', 85000, 4, 'King + Twin', 40, 1, 4.8, 15, 'https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=1200'),
(5, 'Executive Suite', 'executive-suite', 'Our flagship suite with separate living area and executive workspace.', 'Luxury suite with living area', 120000, 3, 'King Bed', 55, 1, 4.9, 22, 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=1200');

INSERT OR IGNORE INTO rooms (room_type_id, room_number, floor, status) VALUES
...
INSERT OR IGNORE INTO room_amenities (room_type_id, amenity_id)
SELECT rt.id, a.id FROM room_types rt, amenities a WHERE rt.id <= 3 AND a.id <= 6;

INSERT OR IGNORE INTO room_amenities (room_type_id, amenity_id)
SELECT rt.id, a.id FROM room_types rt, amenities a WHERE rt.id >= 4;

INSERT OR IGNORE INTO room_images (room_type_id, image_url, alt_text, sort_order) VALUES
...
INSERT OR IGNORE INTO menu_categories (name, slug, sort_order) VALUES
('Breakfast','breakfast',1),('Lunch','lunch',2),('Dinner','dinner',3),
('Drinks','drinks',4),('Coffee','coffee',5),('Desserts','desserts',6);

INSERT OR IGNORE INTO menu_items (category_id, name, description, price, is_chef_recommendation, image_url) VALUES
...
INSERT OR IGNORE INTO event_halls (name, slug, type, capacity, description, base_price, cover_image, amenities_json) VALUES
...
INSERT OR IGNORE INTO event_packages (hall_id, name, description, price, includes_json) VALUES
...
INSERT OR IGNORE INTO event_equipment (name, quantity, daily_rate, status) VALUES
('Projector',5,15000,'available'),('Wireless Microphone',8,8000,'available'),
('LED Stage Lights',4,25000,'available'),('Conference Speaker',6,10000,'available'),
('Folding Tables',40,2000,'available');

INSERT OR IGNORE INTO catering_packages (name, category, description, price_per_person, min_guests, includes_json) VALUES
...
INSERT OR IGNORE INTO gallery_items (category, title, media_url, media_type, sort_order) VALUES
...
INSERT OR IGNORE INTO blog_posts (author_id, title, slug, excerpt, content, cover_image, category, status, views, published_at) VALUES
(1,'Welcome to SUN CITY NYAKARAMBI','welcome-to-sun-city','Discover premium hospitality in Kirehe District.','<p>SUN CITY NYAKARAMBI Ltd welcomes guests to experience comfort, luxury, and exceptional hospitality in Nyakarambi.</p>','https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200','news','published',120,datetime('now')),
(1,'Explore Eastern Province Tourism','eastern-province-tourism','Top attractions near Nyakarambi and Kirehe.','<p>Eastern Province offers lakes, cultural sites, and scenic landscapes perfect for weekend getaways.</p>','https://images.unsplash.com/photo-1523805009345-7448845a9e53?w=1200','tourism','published',85,datetime('now')),
(1,'Hosting Successful Corporate Events','corporate-events-tips','How our conference hall supports professional meetings.','<p>Plan productive workshops and board meetings with our AV-ready conference facilities and catering packages.</p>','https://images.unsplash.com/photo-1511578314322-379afb476865?w=1200','events','published',64,datetime('now'));

INSERT OR IGNORE INTO offers (title, slug, description, discount_percent, coupon_code, offer_type, start_date, end_date, cover_image, is_active) VALUES
('Weekend Escape','weekend-escape','Save on Deluxe Rooms for Friday-Sunday stays.',15,'WEEKEND15','seasonal',date('now'),date('now','+90 days'),'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=1200',1),
('Conference Early Bird','conference-early-bird','Book conference packages 30 days ahead and save.',10,'CONF10','promotion',date('now'),date('now','+120 days'),'https://images.unsplash.com/photo-1511578314322-379afb476865?w=1200',1),
('Wedding Season Package','wedding-season','Exclusive wedding hall and catering bundle.',12,'WED12','package',date('now'),date('now','+180 days'),'https://images.unsplash.com/photo-1511285560929-80b456fea0bc?w=1200',1);

INSERT OR IGNORE INTO testimonials (guest_name, guest_title, content, rating, is_featured, is_approved) VALUES
...
INSERT OR IGNORE INTO cms_pages (slug, title, content, meta_title, meta_description) VALUES
...
INSERT OR IGNORE INTO settings (setting_key, setting_value, category) VALUES
...
INSERT OR IGNORE INTO reviews (user_id, room_type_id, rating, title, content, is_approved) VALUES
...
INSERT OR IGNORE INTO notifications (user_id, title, message, type, link) VALUES
